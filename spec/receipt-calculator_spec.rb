RSpec.describe ReceiptCalculator do
  it "parses CSV input" do
    ...
    expect(receipt.items.size).to eq(4)
    expect(receipt.items[0].product.name).to eq('imported bottle of perfume')
    expect(receipt.items[0].quantity).to eq(1)
    expect(receipt.items[3].product.name).to eq('imported box of chocolates')
    expect(receipt.items[3].quantity).to eq(4)
  end

  it "prints out the receipt details" do
    output = ReceiptCalculator.print_receipt <<~EOS
      Quantity, Product, Price
      1, imported bottle of perfume, 27.99
